...
class ConnectionHandler {
    ...
    public function acceptNewConnections($serverSocket): void {
        ...
        if ($isStreamSocket) {
            ...
            if ($newSocket !== false) {
                ...
                // Add user to the server (welcome messages will be sent after registration)
            ...
            if ($newSocket === false) {
                ...
                // Only show error if it's not error code 0 (which means "success" for non-blocking sockets)
                if ($errorCode !== 0 && $errorCode !== 11 && $errorCode !== 10035) {
                    $this->server->getLogger()->error("Socket accept error: {$errorCode} - {$errorMsg}");
                }
            }
            ...
            // Note: If socket_accept returns false, it means no pending connections
            // This is normal behavior for non-blocking sockets and should not be treated as an error
        }
    }
    ...
    public function handleExistingConnections(): void {
        ...
        foreach ($users as $user) {
            ...
            // Process all available commands for this user
            $commandsProcessed = 0;
            $maxCommandsPerIteration = 10; // Prevent infinite loops

            while ($commandsProcessed < $maxCommandsPerIteration) {
                ...
                if ($data === false) {
                    ...
                    break 2; // Exit both while and foreach loops
                }

                // No complete command available
                if ($data === '') {
                    break; // Exit while loop, continue to next user
                }

                // Update user activity when command is received
                $user->updateActivity();
    ...
    private function processCommand(User $user, string $data): void {
        ...
        // Debug logging
        $this->server->getLogger()->debug("Processing command: {$command} from user " . ($user->getNick() ?? 'unregistered') . " ({$user->getIp()})");
        ...
        if (isset($this->commandHandlers[$command])) {
            ...
            // Unknown command
            $config = $this->server->getConfig();
            $user->send(":{$config['name']} 421 " . ($user->getNick() ?? '*') . " {$command} :Unknown command");
        }
    }
